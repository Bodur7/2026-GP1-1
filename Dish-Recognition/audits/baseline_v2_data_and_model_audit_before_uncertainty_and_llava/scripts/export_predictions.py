...
def main() -> int:
    ...
    parser.add_argument(
        "--mode", choices=["in-domain", "partial-in-domain", "ood"], default="in-domain"
    )
    ...
    if args.mode == "partial-in-domain":
        unknown = sorted(set(dataset.classes) - set(expected_mapping))
        if unknown:
            raise RuntimeError(f"Partial dataset contains classes absent from checkpoint: {unknown}")
        source_to_checkpoint = torch.tensor(
            [expected_mapping[name] for name in dataset.classes], dtype=torch.long
        )
    else:
    ...
    with torch.inference_mode():
        for batch_index, (images, labels) in enumerate(loader, start=1):
            ...
            if source_to_checkpoint is not None:
                labels = source_to_checkpoint[labels]
    ...
    if args.mode in {"in-domain", "partial-in-domain"} and len(logits):
